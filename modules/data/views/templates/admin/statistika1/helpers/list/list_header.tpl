			...
			$('.company_search').keypress(function(event){
                 ...
                 if(str.length >= 2){
                     ...
                     holder = setTimeout(function(){                         
		                ...
		                $.ajax("{$link->getAdminLink('Statistika1')}", {
                            dataType: 'json',
                            data: {
                                ajax: "1",
                                token: "{getAdminToken tab='Statistika1'}",
                                tab: "Statistika1",
                                q: str,
				...
				<td style="vertical-align: bottom;">
					<span style="float: left;">
						...
						<select name="pagination" onchange="submit()">
							...
							{/foreach}
                                <option value="999999"{if $selected_pagination == '999999'} selected="selected" {/if}>{l s='Všetko'}</option>                            
						</select>
								...
								<td {if isset($params.align)} class="{$params.align}" {/if}>
									...
									{else}
										...
										{elseif $params.type == 'select'}
											...
											{/if}
										{else}
                                            {if isset($params.ajax_company_search)}
                                                {if $params.ajax_company_search}
									               <input type="text" class="filter company_search" name="{$table}Filter_{if isset($params.filter_key)}{$params.filter_key}{else}{$key}{/if}" value="{$params.value|escape:'htmlall':'UTF-8'}" {if isset($params.width) && $params.width != 'auto'} style="width:{$params.width}px"{else}style="width:95%"{/if}/>
