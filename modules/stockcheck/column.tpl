...
<link rel="stylesheet" type="text/css" href="{$module_dir}css/thumbnail_slider.css" />
<div style="width:100%; clear:both; position:relative; float:left">
	
	{if isset($productsd) AND $productsd}
	
		<div id="demo3" class="demo">
		
					<ul class="ts_container">
			{foreach from=$productsd item=productd}
           
			    
				
              <li><a href="{$productd.link}" title="{$productd.name|truncate:32:'...'|escape:'htmlall':'UTF-8'}"></a></li>
            ...
            <div class="ts_preview_wrapper">
								<ul class="ts_preview">
                                {foreach from=$productsd item=productd}
                                	<li><img src="{$link->getImageLink($productd.link_rewrite, $productd.id_image, $type)}" alt="{$productd.name|escape:html:'UTF-8'}"  width="{$image1}" /></li>
                                {/foreach}
                                </ul>
                                </div>
					
	{else}
		<p>{l s='No featured products' mod='countdownspecials'}</p>
	{/if}
   ...
   <script type="text/javascript">
			(function($) {
				...
				$.fn.thumbnailSlider.defaults = {
					...
					thumb_width	: {/literal}{$image1}{literal},//your photos width
					thumb_height: {/literal}{$image1}{literal},//your photos height
		...
		<script type="text/javascript">
			$(function() {
				...
				$('#demo3').thumbnailSlider({
					thumb_width	: {/literal}{$image1}{literal},
					thumb_height: {/literal}{$image1}{literal},
					speed		: 200
				});
				//demo4
