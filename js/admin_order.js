...
function addProductRefreshTotal()
{
    ...
    var product_tax_rate = 1 + (parseFloat($('#product_tax_rate').val()) / 100);
	...
	if (use_taxes)
        var total = makeTotalProductCaculation(quantity, price, product_tax_rate);
	else
        var total = makeTotalProductCaculation(quantity, price, 1);

    var provizia = calc_provizia() * quantity;
    $('#add_product_provizia').html(formatCurrency(provizia, currency_format, currency_sign, currency_blank));
...
function editProductRefreshTotal(element)
{
    ...
    var product_tax_rate = parseFloat(element.find('td .product_tax_rate').val());
    ...
    var provizia = (calc_provizia2() * quantity );
	$(myprd).find('.product_provizia').html(formatCurrency(provizia, currency_format, currency_sign, currency_blank));
    

}

function makeTotalProductCaculation(quantity, price , tax)
{
    if('undefined' == typeof tax) var tax = 1;
	return Math.round(quantity * price * tax * 100) / 100;
}
...
{
	$('#total_provisions td.amount').fadeOut('slow', function() {
		$(this).html(formatCurrency(parseFloat(order.total_provisions), currency_format, currency_sign, currency_blank));
		$(this).fadeIn('slow');
	});
...
function calc_provizia()
{
    var cena_2 = parseFloat($('#cena_2').val());
    var provi = parseFloat($('#provizia').val());
    var ws = parseFloat($('#wholesale_price').val());
    var cena =  parseFloat($('#add_product_product_price_tax_excl').val());
    
    if(cena >= cena_2){
        var provizia = cena * provi / 100; 
    } else {
...
function calc_provizia2()
{
    var cp = myprd;
    var cena_2 = parseFloat($(cp).find('.cena_2').val());
    var provi = parseFloat($(cp).find('.provizia').val());
    var ws = parseFloat($(cp).find('.wholesale_price').val());
    var cena =  parseFloat($(cp).find('.edit_product_price_tax_excl').val());
...
//		  console.log(data.provizia);
			...
			$('#cena_2').val(data.cena_2);
			$('#provizia').val(data.provizia);
			$('#wholesale_price').val(data.wholesale_price);
			$('#product_tax_rate').val(data.tax_rate);
            
			$('#add_product_cena_1').html(formatCurrency(data.price, currency_format, currency_sign, currency_blank));
            var cena_2 = data.cena_2;
			$('#add_product_cena_2').html(formatCurrency(cena_2, currency_format, currency_sign, currency_blank));

            var provizia = calc_provizia();
			$('#add_product_provizia').html(formatCurrency(provizia, currency_format, currency_sign, currency_blank));
