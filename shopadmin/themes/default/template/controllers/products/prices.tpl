...
<table>
	<tr>
		<td class="col-left">
			{include file="controllers/products/multishop/checkbox.tpl" field="cena_2" type="default"}
			<label>{l s='Hraničná cena:'}</label>
		</td>
		<td style="padding-bottom:5px;">
			{$currency->prefix}<input size="11" maxlength="14" name="cena_2" id="cena_2" type="text" value="{{toolsConvertPrice price=$product->cena_2}|string_format:'%.2f'}" onchange="this.value = this.value.replace(/,/g, '.');" />{$currency->suffix}
			<p class="preference_description">{l s='Spodná hraničná cena'}</p>
		</td>
	</tr>

	<tr>
		<td class="col-left">
			{include file="controllers/products/multishop/checkbox.tpl" field="provizia" type="default"}
			<label>{l s='Provízia:'}</label>
		</td>
		<td style="padding-bottom:5px;">
			<input size="11" maxlength="14" name="provizia" id="provizia" type="text" value="{{toolsConvertPrice price=$product->provizia}|string_format:'%.2f'}" onchange="this.value = this.value.replace(/,/g, '.');" />&nbsp;%
<!--			<p class="preference_description">{l s='The wholesale price at which you bought this product'}</p> -->
		</td>
	</tr>
