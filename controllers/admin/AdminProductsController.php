...
class AdminProductsControllerCore extends AdminController
{
	...
	public function __construct()
	{
		...
		$this->_select .= 'cl.name `name_category` '.($join_category ? ', cp.`position`' : '').', '.$alias_image.'.`id_image`, '.$alias.'.`price`, '.$alias.'.`wholesale_price`, 0 AS price_final, sav.`quantity` as sav_quantity, '.$alias.'.`active`';
	...
	*/
	...
	public function getPackItems($product = null)
	{
		...
		if (!$product)
		{
			...
			$names = array_unique(explode('Â¤', $names_input));
