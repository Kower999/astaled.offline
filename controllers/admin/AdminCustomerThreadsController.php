...
/*			'contact' => array(
	...
	public function renderList()
	{
	    ...
	    if(Tools::isSubmit('id_order')) {
            $this->_where .= ' AND a.id_order = '.(int)(Tools::getValue('id_order'));
        }
	    ...
	    if(Tools::isSubmit('id_order')) {
          $ordr = (int)(Tools::getValue('id_order'));
		  $params = array(
			$this->l('Total threads') => $all = CustomerThread::getTotalCustomerThreads('id_order = '.$ordr),
			$this->l('Threads pending') => $pending = CustomerThread::getTotalCustomerThreads('status LIKE "%pending%" AND id_order = '.$ordr),
			$this->l('Total customer messages') => CustomerMessage::getTotalCustomerMessages('id_employee = 0 AND id_order = '.$ordr),
			$this->l('Total employee messages') => CustomerMessage::getTotalCustomerMessages('id_employee != 0 AND id_order = '.$ordr),
			$this->l('Unread threads') => $unread = CustomerThread::getTotalCustomerThreads('status = "open" AND id_order = '.$ordr),
