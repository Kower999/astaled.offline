...
{block name="override_tpl"}
	...
	var txt_add_product_stock_issue = "{l s='Pridávate vačšie množstvo ako je na sklade. Ste si istý?' js=1}";
	var txt_add_product_new_invoice = "{l s='Ste si istý že chcete vytvoriť novú faktúru?' js=1}";
	var txt_add_product_no_product = "{l s='Chyba: Žiadny produkt nebol zvolený' js=1}";
	var txt_add_product_no_product_quantity = "{l s='Chyba: Množstvo produktu musí byť nastavené' js=1}";
	var txt_add_product_no_product_price = "{l s='Chyba: Cena produktu musí byť nastavená' js=1}";
	var txt_confirm = "{l s='Ste si istý?' js=1}";
	...
	<div class="button-command">
			{if (count($invoices_collection))}
				<a class="button" href="{$link->getAdminLink('AdminPdf')|escape:'htmlall':'UTF-8'}&submitAction=generateInvoicePDF&id_order={$order->id}" target="_blank">
					<img src="../img/admin/charged_ok.gif" alt="{l s='Zobraziť faktúru'}" /> {l s='Zobraziť faktúru'}
				</a>
			{else}
				<img src="../img/admin/charged_ko.gif" alt="{l s='Žiadna faktúra'}" /> {l s='Žiadna faktúra'}
			{/if}
			 |
			{if (($currentState && $currentState->delivery) || $order->delivery_number)}
				<a class="button"  href="{$link->getAdminLink('AdminPdf')|escape:'htmlall':'UTF-8'}&submitAction=generateDeliverySlipPDF&id_order={$order->id}" target="_blank">
					<img src="../img/admin/delivery.gif" alt="{l s='Zobraziť dodací list'}" /> {l s='Zobraziť dodací list'}
				</a>
			{else}
				<img src="../img/admin/delivery_ko.gif" alt="{l s='Žiadny dodací list'}" /> {l s='Žiadny dodací list'}
			{/if}
			 |
			<a class="button" href="javascript:window.print()"><img src="../img/admin/printer.gif" alt="{l s='Vytlačiť objednávku'}" title="{l s='Vytlačiť objednávku'}" /> {l s='Vytlačiť objednávku'}</a>
		</div>
		<div class="metadata-command">
			...
			|</dl>
			<dl>
				<dt><a href="{$link->getAdminLink('AdminCustomerThreads')|escape:'htmlall':'UTF-8'}">{l s='Správy pre zákazníka:'}</a></dt>
			...
			|</dl>
			<dl>
				<dt>{l s='Spolu k úhrade:'}</dt>
				<dd class="total_paid">{displayPrice price=$order->total_paid_tax_incl currency=$currency->id}</dd>
			</dl>
			<dl>
				<dt>{l s='Splatnosť:'}</dt>
                <input type="text" name="date_pay" id="date_pay" class="datepicker" value="{dateFormat date=$order->date_pay full=false}" onchange="setDatePay();"/>
	...
	<div class="container-command">
		...
		<div style="width: 49%; float:left;">
			...
			<form action="{$currentIndex}&vieworder&token={$smarty.get.token}" method="post">
				...
				<input type="submit" name="submitState" value="{l s='Pridať'}" class="button" />
			</form>
            {/if} 
			<br />
			...
			<br />
			<fieldset>
				<legend><img src="../img/admin/tab-customers.gif" /> {l s='Informácie o zákazníkovy'}</legend>
				...
				{if ($customer->isGuest())}
					{l s='Táto objednávka bola zadaná návštevníkom.'}
					{if (!Customer::customerExists($customer->email))}
					<form method="post" action="index.php?tab=AdminCustomers&id_customer={$customer->id}&token={getAdminToken tab='AdminCustomers'}">
						<input type="hidden" name="id_lang" value="{$order->id_lang}" />
						<p class="center"><input class="button" type="submit" name="submitGuestToCustomer" value="{l s='Transformovať hosťa na zákazníka'}" /></p>
						{l s='Táto možnost vygeneruje náhodné heslo a pošle email zákazníkovy.'}
					</form>
					{else}
						<div><b style="color:red;">{l s='Už existuje registrovaný zákazník s touto emailovou adresou.'}</b></div>
					{/if}
				{else}
					{l s='Účet registrovaný:'} <b>{dateFormat date=$customer->date_add full=true}</b><br />
					{l s='Platné objednávky:'} <b>{$customerStats['nb_orders']}</b><br />
					{l s='Celková tržba od registrácie:'} <b>{displayPrice price=Tools::ps_round(Tools::convertPrice($customerStats['total_orders'], $currency), 2) currency=$currency->id}</b><br />
			</fieldset>
				{/if}
			{/if}
		...
		<div style="width: 49%; float:right;">
			<div class="button-command-prev-next">
				<b>{l s='Objednávky'}</b> :
				{if $previousOrder}<a class="button" href="{$link->getAdminLink('AdminOrders')|escape:'htmlall':'UTF-8'}&vieworder&id_order={$previousOrder}">{l s='< Predch.'}</a>{/if}
				{if $nextOrder}<a class="button" href="{$link->getAdminLink('AdminOrders')|escape:'htmlall':'UTF-8'}&vieworder&id_order={$nextOrder}">{l s='Nasl. >'}</a>{/if}
			</div>
			<div class="clear"></div>
			...
			<!-- Documents block -->
			<fieldset>
				<legend><img src="../img/admin/details.gif" /> {l s='Dokumenty'}</legend>

				{* Include document template *}
				{if $overiden_documents}                
				    {include file=$overiden_documents}
                {else}
			...
			<fieldset>
				<legend><img src="../img/admin/money.gif" /> {l s='Platby'}</legend>
				...
				<p class="error" style="{if round($order->total_paid_tax_incl, 2) == round($total_paid, 2) || $currentState->id == 6}display: none;{/if}">
					{l s='Varovanie:'} {displayPrice price=$total_paid currency=$currency->id}
					{l s='zaplatené namiesto '} <span class="total_paid">{displayPrice price=$order->total_paid_tax_incl currency=$currency->id}</span>
					
					{foreach $order->getBrother() as $brother_order}
						{if $brother_order@first}
							{if count($order->getBrother()) == 1}
								<br />{l s='Toto varovanie tiež platí pre objednávku '}
							{else}
								<br />{l s='Toto varovanie tiež platí pre objednávky:'}
							{/if}
						{/if}
						<a href="{$current_index}&vieworder&id_order={$brother_order->id}&token={$smarty.get.token|escape:'htmlall':'UTF-8'}">#{'%06d'|sprintf:$brother_order->id}</a>
					{/foreach}
					...
					<table class="table" width="100%" cellspacing="0" cellpadding="0">
						...
						<tbody>
							...
							<tr class="current-edit">
								...
								{/if}
								<td><input class="button" type="submit" name="submitAddPayment" value="{l s='Pridať'}" /></td>
							</tr>
						</tbody>
					</table>
				</form>
			...
			{if !$order->isVirtual()}
				<fieldset>
					<legend><img src="../img/admin/delivery.gif" /> {l s='Doprava'}</legend>

					<div class="clear" style="float: left; margin-right: 10px;">
						<span>{l s='Recyklované balenie:'}</span>
						{if $order->recyclable}
					...
					<div style="float: left;">
						<span>{l s='Darčekové balenie:'}</span>
						{if $order->gift}
						<img src="../img/admin/enabled.gif" />
						</div>
						<div style="clear: left; margin: 0px 42px 0px 42px; padding-top: 2px;">
							{if $order->gift_message}
							<div style="border: 1px dashed #999; padding: 5px; margin-top: 8px;"><b>{l s='Správa:'}</b><br />{$order->gift_message|nl2br}</div>
							{/if}
						{else}
						<img src="../img/admin/disabled.gif" />
						{/if}
				...
				<!-- Return block -->
				<fieldset>
					<legend><img src="../img/admin/delivery.gif" /> {l s='Návratka'}</legend>
								...
								<td>
									...
									<form style="display: inline;" method="post" action="{$link->getAdminLink('AdminOrders')|escape:'htmlall':'UTF-8'}&vieworder&id_order={$order->id}&id_order_invoice={if $line.id_order_invoice}{$line.id_order_invoice|escape:'htmlall':'UTF-8'}{else}0{/if}&id_carrier={if $line.id_carrier}{$line.id_carrier|escape:'htmlall':'UTF-8'}{else}0{/if}">
										<span class="shipping_number_edit" style="display:none;">
											...
											<input type="submit" class="button" name="submitShippingNumber" value="{l s='Aktualizovať'}" />
										</span>
										<a href="#" class="edit_shipping_number_link"><img src="../img/admin/edit.gif" alt="{l s='Upraviť'}" /></a>
										<a href="#" class="cancel_shipping_number_link" style="display: none;"><img src="../img/admin/disabled.gif" alt="{l s='Zrušiť'}" /></a>
									</form>
									{/if}
								</td>
							</tr>
	...
	<div class="container-command container-command-top-spacing">
		...
		{if !$order->isVirtual()}
			<div style="width: 49%; float:left;">
				<!-- Shipping address -->
				<fieldset>
					<legend><img src="../img/admin/delivery.gif" alt="{l s='Adresa doručenia'}" />{l s='Adresa doručenia'}</legend>
					...
					<form method="post" action="{$link->getAdminLink('AdminOrders')|escape:'htmlall':'UTF-8'}&vieworder&id_order={$order->id}">
						<div style="margin-bottom:5px;">
							<p>
								...
								<input class="button" type="submit" name="submitAddressShipping" value="{l s='Zmeniť'}" />
							</p>
						</div>
					</form>
					{/if}
...
{*}					{if $addresses.delivery->other}<hr />{$addresses.delivery->other}<br />{/if} {*}
		...
		<div style="width: 49%; float:right;">
			<!-- Invoice address -->
			<fieldset>
				<legend><img src="../img/admin/invoice.gif" alt="{l s='Fakturačná adresa'}" />{l s='Fakturačná adresa'}</legend>
				...
				<form method="post" action="{$link->getAdminLink('AdminOrders')|escape:'htmlall':'UTF-8'}&vieworder&id_order={$order->id}">
					<div style="margin-bottom:5px;">
						<p>
							...
							<input class="button" type="submit" name="submitAddressInvoice" value="{l s='Zmeniť'}" />
						</p>
					</div>
				</form>
				{/if}
	...
	<form style="width: 98%" class="container-command-top-spacing" action="{$current_index}&vieworder&token={$smarty.get.token}&id_order={$order->id}" method="post" onsubmit="return orderDeleteProduct('{l s='Tento produkt sa nedá vrátiť'}', '{l s='Množstvo pre zrušenie je vačsie ako dostupné'}');">
		<input type="hidden" name="id_order" value="{$order->id}" />
		<fieldset style="width: 100%; ">
			<div style="display: none">
				<input type="hidden" value="{$order->getWarehouseList()|implode}" id="warehouse_list" />
			</div>
			<legend><img src="../img/admin/cart.gif" alt="{l s='Produkty'}" />{l s='Produkty'}</legend>
			<div style="float:left;width: 100%;">
				{if $can_edit}
				<div style="float: left;"><a href="#" class="add_product button"><img src="../img/admin/add.gif" alt="{l s='Pridať produkt'}" /> {l s='Pridať produkt'}</a></div>
				<div style="float: right; margin-right: 10px" id="refundForm">
				<!--
					<a href="#" class="standard_refund"><img src="../img/admin/add.gif" alt="{l s='Štandardné vrátenie peňazí'}" /> {l s='Štandardné vrátenie peňazí'}</a>
					<a href="#" class="partial_refund"><img src="../img/admin/add.gif" alt="{l s='Čiastočné vrátenie peňazí'}" /> {l s='Čiastočné vrátenie peňazí'}</a>
				-->
				</div>
				<br clear="left" /><br />
				{/if}
				<table style="width: 100%;" cellspacing="0" cellpadding="0" class="table" id="orderProducts">
					<tr>
						...
						<th>{l s='EAN'}</th>
						<th>{l s='Produkt'}</th>
						<th style="width: 15%; text-align: center">{l s='Jedn. cena (bez DPH)'} <sup>*</sup></th>
						<th style="width: 15%; text-align: center">{l s='Jedn. cena'} <sup>*</sup></th>
						<th style="width: 15%; text-align: center">{l s='Hraničná Cena'} <sup>*</sup></th>
						<th style="width: 15%; text-align: center">{l s='Provízia'} <sup>*</sup></th>
						<th style="width: 4%; text-align: center">{l s='Množstvo'}</th>
						{if ($order->hasBeenPaid())}<th style="width: 3%; text-align: center">{l s='Preplatené'}</th>{/if}
						{if ($order->hasBeenDelivered() || $order->hasProductReturned())}<th style="width: 3%; text-align: center">{l s='Vrátené'}</th>{/if}
						{if $stock_management}<th style="width: 10%; text-align: center">{l s='Dostupné'}</th>{/if}
						<th style="width: 10%; text-align: center">{l s='Spolu'} <sup>*</sup></th>
						<th colspan="2" style="display: none;" class="add_product_fields">&nbsp;</th>
						<th colspan="2" style="display: none;" class="edit_product_fields">&nbsp;</th>
						<th colspan="2" style="display: none;" class="standard_refund_fields"><img src="../img/admin/delete.gif" alt="{l s='Produkty'}" />
				...
				<div style="float:left; width:280px; margin-top:15px;">
					<sup>*</sup> {l s='Pre túto skupinu zákazníkov sa ceny zobrazujú :'}
				...
				<div style="float:right; margin-top: 20px;">
					<table class="table" width="450px;" style="border-radius:0px;"cellspacing="0" cellpadding="0">
						...
						<tr id="total_provisions">
							<td width="150px;"><b>{l s='Provízia'}</b></td>
							<td class="amount" align="right">{displayPrice price=$order->total_provisions currency=$currency->id}</td>
							<td class="partial_refund_fields current-edit" style="display:none;">&nbsp;</td>
						</tr>
						...
						<tr style="font-size: 20px" id="total_order">
							<td style="font-size: 20px">{l s='Spolu'}</td>
				...
				<div style="float:right; width:450px; margin-top:15px;">
					<table cellspacing="0" cellpadding="0" class="table" style="width:100%;">
						<tr>
							<th><img src="../img/admin/coupon.gif" alt="{l s='Zľavy'}" />{l s='Názov zľavy'}</th>
							<th align="center" style="width: 100px">{l s='Hodnota'}</th>
							{if $can_edit}<th align="center" style="width: 30px">{l s='Akcia'}</th>{/if}
						</tr>
						...
						<tr>
							...
							{if $can_edit}
							<td class="center">
								<a href="{$current_index}&submitDeleteVoucher&id_order_cart_rule={$discount['id_order_cart_rule']}&id_order={$order->id}&token={$smarty.get.token|escape:'htmlall':'UTF-8'}"><img src="../img/admin/delete.gif" alt="{l s='Zmazať kupón'}" /></a>
							</td>
							{/if}
						</tr>
						{/foreach}
					{if $can_edit}
						<tr>
							<td colspan="3" class="center">
								<a class="button" href="#" id="add_voucher"><img src="../img/admin/add.gif" alt="{l s='Pridať'}" /> {l s='Pridať novú zľavu'}</a>
							</td>
						</tr>
			...
			<div style="float: right; width: 160px; display: none;" class="standard_refund_fields">
				{if ($order->hasBeenDelivered() && Configuration::get('PS_ORDER_RETURN'))}
					<input type="checkbox" name="reinjectQuantities" class="button" />&nbsp;<label for="reinjectQuantities" style="float:none; font-weight:normal;">{l s='Naskladniť produkt'}</label><br />
				{/if}
				{if ((!$order->hasBeenDelivered() && $order->hasBeenPaid()) || ($order->hasBeenDelivered() && Configuration::get('PS_ORDER_RETURN')))}
					<input type="checkbox" id="generateCreditSlip" name="generateCreditSlip" class="button" onclick="toggleShippingCost(this)" />&nbsp;<label for="generateCreditSlip" style="float:none; font-weight:normal;">{l s='Vygenerovať dobropis'}</label><br />
					<input type="checkbox" id="generateDiscount" name="generateDiscount" class="button" onclick="toggleShippingCost(this)" />&nbsp;<label for="generateDiscount" style="float:none; font-weight:normal;">{l s='Vygenerovať kupón'}</label><br />
					<span id="spanShippingBack" style="display:none;"><input type="checkbox" id="shippingBack" name="shippingBack" class="button" />&nbsp;<label for="shippingBack" style="float:none; font-weight:normal;">{l s='Preplaťiť poštovné'}</label><br /></span>
				{/if}
				{if (!$order->hasBeenDelivered() || ($order->hasBeenDelivered() && Configuration::get('PS_ORDER_RETURN')))}
					<div style="text-align:center; margin-top:5px;">
						<input type="submit" name="cancelProduct" value="{if $order->hasBeenDelivered()}{l s='Vrátiť produkty'}{elseif $order->hasBeenPaid()}{l s='Preplaťiť produkty'}{else}{l s='Zrušiť produkty'}{/if}" class="button" style="margin-top:8px;" />
					</div>
				{/if}
			</div>
			<div style="float: right; width: 160px; display:none;" class="partial_refund_fields">
				<input type="checkbox" name="reinjectQuantities" class="button" />&nbsp;<label for="reinjectQuantities" style="float:none; font-weight:normal;">{l s='Naskladniť produkty'}</label><br />
				<input type="checkbox" id="generateDiscountRefund" name="generateDiscountRefund" class="button" onclick="toggleShippingCost(this)" />&nbsp;<label for="generateDiscount" style="float:none; font-weight:normal;">{l s='Vygenerovať kupón'}</label><br />
				<input type="submit" name="partialRefund" value="{l s='Čiastočne preplatiť'}" class="button" style="margin-top:8px;" />
			</div>
		</fieldset>
	</form>
	...
	<div style="float: left">
		<form action="{$smarty.server.REQUEST_URI}&token={$smarty.get.token}" method="post" onsubmit="if (getE('visibility').checked == true) return confirm('{l s='Chcete poslať túto správu zákazníkovy?'}');">
		<fieldset style="width: 400px;">
			<legend style="cursor: pointer;" onclick="$('#message').slideToggle();$('#message_m').slideToggle();return false"><img src="../img/admin/email_edit.gif" /> {l s='Nová správa'}</legend>
			<div id="message_m" style="display: {if Tools::getValue('message')}none{else}block{/if}; overflow: auto; width: 400px;">
				<a href="#" onclick="$('#message').slideToggle();$('#message_m').slideToggle();return false"><b>{l s='Klikni sem'}</b> {l s='pre pridanie komentára alebo poslanie správy zákazníkovy'}</a>
			</div>
			<a href="{$link->getAdminLink('AdminCustomerThreads')|escape:'htmlall':'UTF-8'}"><b>{l s='Klikni sem'}</b> {l s='pre zobrazenie všetkých správ'}</a><br>
			<div id="message" style="display: {if Tools::getValue('message')}block{else}none{/if}">
						<select name="order_message" id="order_message" onchange="orderOverwriteMessage(this, '{l s='Chcete prepísať vašu existujúcu správu?'}')">
							<option value="0" selected="selected">-- {l s='Zvoľte štandardnú správu'} --</option>
			{foreach from=$orderMessages item=orderMessage}
				<option value="{$orderMessage['message']|escape:'htmlall':'UTF-8'}">{$orderMessage['name']}</option>
			{/foreach}
						</select><br /><br />
						<b>{l s='Zobraziť zákazníkovy?'}</b>
						<input type="radio" name="visibility" id="visibility" value="0" /> {l s='Áno'}
						<input type="radio" name="visibility" value="1" checked="checked" /> {l s='Nie'}
						<p id="nbchars" style="display:inline;font-size:10px;color:#666;"></p><br /><br />
				<textarea id="txt_msg" name="message" cols="50" rows="8" onKeyUp="var length = document.getElementById('txt_msg').value.length; if (length > 600) length = '600+'; document.getElementById('nbchars').innerHTML = '{l s='600 znakov max'} (' + length + ')';">{Tools::getValue('message')|escape:'htmlall':'UTF-8'}</textarea><br /><br />
				<input type="hidden" name="id_order" value="{$order->id}" />
				<input type="hidden" name="id_customer" value="{$order->id_customer}" />
				<input type="submit" class="button" name="submitMessage" value="{l s='Poslať'}" />
			</div>
		</fieldset>
		</form>
	...
	{if (sizeof($messages))}
		<br />
		<fieldset style="width: 400px;">
		<legend><img src="../img/admin/email.gif" /> {l s='Správy'}</legend>
		{foreach from=$messages item=message}
			<div style="overflow:auto; width:400px;" {if $message['is_new_for_me']}class="new_message"{/if}>
			{if ($message['is_new_for_me'])}
				<a class="new_message" title="{l s='Označiť túto správu ako \'zobrazenú\''}" href="{$smarty.server.REQUEST_URI}&token={$smarty.get.token}&messageReaded={$message['id_message']}"><img src="../img/admin/enabled.gif" alt="" /></a>
			{/if}
			{l s='Dátum'} <i>{dateFormat date=$message['date_add']}
			</i> {l s='od'} <b>{if ($message['elastname'])}{$message['efirstname']} {$message['elastname']}{else}{$message['cfirstname']} {$message['clastname']}{/if}</b>
			{if ($message['private'] == 1)}<span style="color:red; font-weight:bold;">{l s='Súkromná:'}</span>{/if}
	...
	<div class="clear">&nbsp;</div>
	<br /><br /><a href="{$current_index}&token={$smarty.get.token}"><img src="../img/admin/arrow2.gif" /> {l s='Spať na zoznam'}</a><br />
{/block}
