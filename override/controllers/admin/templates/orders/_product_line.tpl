...
{else}
	...
	{assign var=product_price_tax_excl value=$product['unit_price_tax_excl']}
{/if}


{assign var=total_price_tax_incl value=$product['total_price_tax_incl']}

{assign var=product_tax_rate value=(1 + ($product['tax_calculator']->taxes[0]->rate / 100))}

{assign var=product_price2 value=$product['cena_2']}
{if ($product['unit_price_tax_excl'] >= $product['cena_2'])}
    {assign var=product_provizia value=($product['unit_price_tax_excl'] * $product['product_quantity'] * ($product['provizia'] / 100))}
{else}
    {assign var=product_provizia value=(($product['unit_price_tax_excl'] - $product['wholesale_price']) * $product['product_quantity'] / 2)}
{/if}
...
<tr{if isset($product.image) && $product.image->id && isset($product.image_size)} height="{$product['image_size'][1] + 7}"{/if}>
	...
	<td align="center">{if isset($product.ean13)}{$product.ean13}{/if}</td>    
	...
	<td align="center">
		<span class="product_price_show">{displayPrice price=$product_price_tax_excl currency=$currency->id}</span>
	</td>
	<td align="center">
	...
	<td align="center">
		<span>{displayPrice price=$product_price2 currency=$currency->id}</span>
	</td>
	<td align="center">
		<span class="product_provizia">{displayPrice price=$product_provizia currency=$currency->id}</span>
        <input type="hidden" class="cena_2" value="{$product['cena_2']}" />         
        <input type="hidden" class="provizia" value="{$product['provizia']}" />         
        <input type="hidden" class="wholesale_price" value="{$product['wholesale_price']}" />                 
        <input type="hidden" class="product_tax_rate" value="{$product_tax_rate}" />                 
	...
	<td align="center" class="total_product">
		{displayPrice price=(Tools::ps_round($total_price_tax_incl, 2)) currency=$currency->id}
	</td>
