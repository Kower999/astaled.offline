...
<script type="text/javascript">
	...
	var id_currency = '';
	var id_lang = '';
	var txt_show_carts = '{l s='Zobraziť košíky a objednávky'}';
	var txt_hide_carts = '{l s='Skryť košíky a objednávky'}';
			...
			success : function(res)
			{
				if(res.found)
				{
					...
					$.each(res.customers, function() {
						html += '<li class="customerCard"><div class="customerName"><a class="fancybox" href="{$link->getAdminLink('AdminCustomers')}&id_customer='+this.id_customer+'&viewcustomer&liteDisplaying=1">'+this.company+'</a></div>';
                        html += '<div class="customerName"><a class="fancybox" href="{$link->getAdminLink('AdminCustomers')}&id_customer='+this.id_customer+'&viewcustomer&liteDisplaying=1">'+this.firstname+' '+this.lastname+'</a><span class="customerBirthday"> '+((this.birthday != '0000-00-00') ? this.birthday : '')+'</span></div>';
		...
		$.ajax({
			...
			success : function(res)
			{
				...
				if(res.found)
				{
					...
					$('#products_found').show();
					products_found += '<label>{l s='Produkty:'}</label><select id="id_product" onclick="display_product_attributes();display_product_customizations();">';
...
<fieldset id="customer_part">
	<legend><img src="../img/admin/tab-customers.gif" />{l s='Zákazník'}</legend>
	<label>{l s='Hľadať zákazníka:'}</label>
	<div class="margin-form">
		<input type="text" id="customer" value="" />
		<p>{l s='Hľadať zákazníka napísaním prvých písmen (min 2)'}</p>
		<a class="fancybox button" href="{$link->getAdminLink('AdminCustomers')|escape:'htmlall':'UTF-8'}&addcustomer&liteDisplaying=1&submitFormAjax=1#">
			<img src="../img/admin/add.gif" title="new"/><span>{l s='Pridať nového zákazníka'}</span>
		</a>
	</div>
	<div id="customers">
	</div>
...
<fieldset id="products_part" style="display:none;"><legend><img src="../img/t/AdminCatalog.gif" />{l s='Košík'}</legend>
	<div>
		<label>{l s='Hľadať produkt:'} </label>
		<div class="margin-form">
			...
			<p>{l s='Hladať produkt napísaním prvých písmen (min 2)'}</p>
		</div>
		<div id="products_found">
			...
			<p {if $nos_qty2add}{else}style="display: none;"{/if}><label for="qty">{l s='Množstvo:'}</label><input type="text" name="qty" id="qty" value="1" />{*}&nbsp;<b>{l s='Na sklade:'}</b>&nbsp;<span id="qty_in_stock"></span>{*}</p>
			<div class="margin-form">
				<p><input type="submit" onclick="addProduct();return false;" class="button" id="submitAddProduct" value="{l s='Pridať do košíka'}"/></p>
			</div>
		</div>
	</div>
	...
	<div>
		<table cellspacing="0" cellpadding="0" class="table widthfull" id="customer_cart">
			...
			<thead>
				<tr>
					<th height="39px">{l s='Obr.'}</th>
					<th>{l s='EAN'}</th>
					<th>{l s='Produkt'}</th>
					<th>{l s='Kód'}</th>
					<th>{l s='Jedn. cena'}</th>
					<th style="width: 80px;">{l s='Množstvo'}</th>
					<th style="width: 80px;">{l s='Cena'}<br />{l s='(bez DPH)'}</th>
					<th style="width: 80px;">{l s='Cena'}<br />{l s='(s DPH)'}</th>
					<th style="width: 80px;">{l s='Odporúčaná cena'}<br />{l s='(bez DPH)'}</th>
					<th style="width: 80px;">{l s='Hraničná cena'}<br />{l s='(bez DPH)'}</th>
                    {if $nos_provisions}
					   <th style="width: 80px;">{l s='Provízia'}</th>
                    {/if}
				</tr>
			</thead>
			<tbody>
	...
	<div>
		<p><label for="id_currency">{l s='Mena:'}</label>
			<script type="text/javascript">
		...
		<p>
		<label for="id_lang">{l s='Jazyk:'}</label>
			...
			<div id="nonOrderedCarts">
				...
				<thead>
					<tr>
						...
						<th class="left">{l s='Dátum'}</th>
						<th class="left">{l s='Total'}</th>
						<th class="left">{l s='Akcia'}</th>
					</tr>
					</thead>
					<tbody>
					</tbody>
				...
				<table cellspacing="0" cellpadding="0" class="table  width5">
					...
					<thead>
						<tr>
							<th height=39px" class="left">{l s='ID'}</th>
							<th class="left">{l s='Dátum'}</th>
							<th class="left">{l s='Produkty'}</th>
							<th class="left">{l s='Total zaplatené'}</th>
							<th class="left">{l s='Platba'}</th>
							<th class="left">{l s='Stav'}</th>
							<th class="left">{l s='Akcia'}</th>
						</tr>
					</thead>
					<tbody>
...
<fieldset id="vouchers_part" style="display:none;">
	<legend><img src="../img/t/AdminCartRules.gif" />{l s='Kupóny'}</legend>
	<p>
		<label>{l s='Hľadať kupón:'} </label>
		<input type="text" id="voucher" value="" />
		<a class="fancybox button" href="{$link->getAdminLink('AdminCartRules')|escape:'htmlall':'UTF-8'}&addcart_rule&liteDisplaying=1&submitFormAjax=1#"><img src="../img/admin/add.gif" title="new"/>{l s='Pridať nový kupón'}</a>
	</p>
	<div class="margin-form">
		<table cellspacing="0" cellpadding="0" class="table" id="voucher_list">
			<thead>
				<tr>
					<th class="left">{l s='Názov'}</th>
					<th class="left">{l s='Popis'}</th>
					<th class="left">{l s='Hodnota'}</th>
					<th class="left">{l s='Akcia'}</th>
				</tr>
			</thead>
			<tbody>
...
<fieldset id="address_part" style="display:none;">
	<legend><img src="../img/t/AdminAddresses.gif" />{l s='Adresy'}</legend>
	<div id="addresses_err" class="warn" style="display:none;"></div>
	<div id="address_delivery">
		<h3>{l s='Adresa Doručenia:'}</h3>
	...
	<div id="address_invoice">
		<h3>{l s='Fakturačná Adresa:'}</h3>
		<select id="id_address_invoice" name="id_address_invoice">
		</select>
		<div id="address_invoice_detail">
		</div>
	</div>
<a class="fancybox button" id="new_address" href="{$link->getAdminLink('AdminAddresses')|escape:'htmlall':'UTF-8'}&addaddress&id_customer=42&liteDisplaying=1&submitFormAjax=1#"><img src="../img/admin/add.gif" title="new"/>{l s='Pridať novú adresu'}</a>
</fieldset>
<br />
<fieldset id="carriers_part" style="display:none;">
	<legend><img src="../img/t/AdminCarriers.gif" />{l s='Doprava'}</legend>
	<div id="carriers_err" style="display:none;" class="warn"></div>
	<div id="carrier_form">
		<div>
			<p>
				<label>{l s='Možnosti dopravy:'} </label>
				<select name="delivery_option" id="delivery_option">
				</select>
			</p>
			<p>
				<label for="shipping_price">{l s='Cena dopravy:'}</label> <span id="shipping_price"  name="shipping_price"></span>&nbsp;<span class="currency_sign"></span>&nbsp;
			</p>
			<p>
				<label for="free_shipping">{l s='Doprava zdarma:'}</label>
		...
		<div id="float:left;">
			...
			{/if}
			{if $gift_wrapping}
				<p><input type="checkbox" name="order_gift" id="order_gift" value="1" /> <label for="order_gift">{l s='Darček'}</label></p>
				<p><label for="gift_message">{l s='Darčeková správa:'}</label><textarea id="gift_message" cols="40" rows="4"></textarea></p>
			{/if}
		</div>
	</div>
</fieldset>
...
<fieldset id="summary_part" style="display:none;">
	<legend><img src="../img/t/AdminPayment.gif" />{l s='Sumár'}</legend>
	<div id="send_email_feedback"></div>
	<div id="cart_summary" style="clear:both;float:left;">
		<ul>
			<li><span class="total_cart">{l s='Spolu za produkty:'}</span><span id="total_products"></span><span class="currency_sign"></span></li>
			<li><span class="total_cart">{l s='Spolu za kupóny:'}</span><span id="total_vouchers"></span><span class="currency_sign"></span></li>
			<li><span class="total_cart">{l s='Spolu za dopravu:'}</span><span id="total_shipping"></span><span class="currency_sign"></span></li>
			<li><span class="total_cart">{l s='Spolu DPH:'}</span><span id="total_taxes"></span><span class="currency_sign"></span></li>
			<li><span class="total_cart">{l s='Spolu bez DPH:'}</span><span id="total_without_taxes"></span><span class="currency_sign"></span></li>
			<li><span class="total_cart">{l s='Spolu s DPH:'}</span><span id="total_with_taxes"></span><span class="currency_sign"></span></li>
			<li {if !$nos_provisions}style="display: none;"{/if}><span class="total_cart">{l s='Provízia za objednávku:'}</span><span id="total_provisions"></span>&nbsp;&nbsp;<span class="currency_sign"></span></li>
		</ul>
	</div>
	<div class="order_message_right">
		<label for="order_message">{l s='Správa k objednávke:'}</label>
		<div class="margin-form">
...
-->        
		...
		<label>{l s='Typ objednávky:'}</label>
		<div class="margin-form">
...
<!--            <input type="text" name="date_pay" id="date_pay" class="datepicker" /> -->
		...
		<div class="margin-form">
			<input type="submit" name="submitAddOrder" class="button" value="{l s='Vytvoriť objednávku'}" />
		</div>
	</div>
</fieldset>
</form>
