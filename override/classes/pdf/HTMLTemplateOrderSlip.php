...
class HTMLTemplateOrderSlip extends HTMLTemplateOrderSlipCore
{
	...
	public function __construct(OrderSlip $order_slip, $smarty)
	{
		...
		// header informations
		$this->date = Tools::displayDate($this->order_slip->date_add,$id_lang);
		$this->title = HTMLTemplateOrderSlip::l('Slip #').Configuration::get('PS_CREDIT_SLIP_PREFIX', $id_lang).sprintf('%06d', (int)$this->order_slip->id);
		...
		$this->smarty->assign(array(
            ...
            'vs' => Configuration::get('PS_CREDIT_SLIP_PREFIX', Context::getContext()->language->id, null, (int)$this->order->id_shop) . sprintf('%06d',(int)$this->order_slip->id),
