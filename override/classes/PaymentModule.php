		...
		if (Validate::isLoadedObject($this->context->cart) && $this->context->cart->OrderExists() == false)
		{
			...
			foreach ($package_list as $id_address => $packageByAddress)
				foreach ($packageByAddress as $id_package => $package)
				{
                    ...
                    if(!empty($orderr)){
                        $order->date_add = $orderr->date_add;
                        $order->date_upd = $orderr->date_upd;
                    }

					// Creating order
					$result = $order->add($autodate);
...
// kontrola zaplatenej sumy vypnuta akokolvek dame status ktory tam ma byt
//var_dump($cart_total_paid);
